              ...
              <div class="order-health-chart">
                ...
                <div class="donut-legend-compact">
                  <div class="legend-item-compact">
                    <span class="legend-dot" style="background: #0f9d6b"></span
                    >{{ t("status.delivered") }}
                  </div>
                  <div class="legend-item-compact">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="item in backlogItems" :key="item.id">
                  <td @click="showBacklogDetail(item)" style="cursor: pointer" class="cell-id">
                    {{ item.order_id }}
                  </td>
                  <td @click="showBacklogDetail(item)" style="cursor: pointer" class="cell-id">
                    {{ item.item_sku }}
...
export default {
  ...
  setup() {
    ...
    const orderHealthMetrics = computed(() => {
      ...
      deliveredOrders.forEach((o) => {
        if (o.order_date && o.actual_delivery) {
          const days = diffCalendarDays(o.order_date, o.actual_delivery);
          totalDays += days ?? 0;
          if (days !== null) countWithDates++;
        }
      });
    ...
    const categoryData = computed(() => {
      ...
      // Use accent color for all category bars (design token via inline style)
      ...
      if (Array.isArray(allOrders.value)) {
        allOrders.value.forEach((order) => {
          if (order && order.order_date) {
            const ym = getYearMonthFromISO(order.order_date);
            if (ym) {
    ...
    const topProducts = computed(() => {
      ...
      return Object.values(productMap)
        .sort((a, b) => {
          const cmp = compareISO(
            a.firstOrderDate || "9999-12-31",
            b.firstOrderDate || "9999-12-31",
          );
          if (cmp !== 0) return cmp;
    ...
    const backlogItems = computed(() => allBacklogItems.value);
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [summaryData, inventoryData, ordersData, backlogData] =
          await Promise.all([
            api.getDashboardSummary(filters, { signal: controller.signal }),
            api.getInventory(filters, { signal: controller.signal }),
            api.getOrders(filters, { signal: controller.signal }),
            api.getBacklog(filters, { signal: controller.signal }),
          ]);
        ...
        error.value = err?.userMessage || "Failed to load dashboard data";
      } finally {
        loading.value = false;
      }
    };
...
.kpi-card {
  ...
  animation: fadeRise 350ms var(--ease) both;
}
