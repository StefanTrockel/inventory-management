...
export default {
  ...
  setup() {
    ...
    const getStockStatusKey = (item) => {
      ...
      if (item.quantity_on_hand <= item.reorder_point * 1.5) return "adequate";
      return "inStock";
    };
    ...
    const filteredItems = computed(() => {
      ...
      return filtered.slice().sort((a, b) => {
        return (
          STATUS_ORDER[getStockStatusKey(a)] -
          STATUS_ORDER[getStockStatusKey(b)]
        );
      });
    });
    ...
    const getStockStatus = (item) => t(`status.${getStockStatusKey(item)}`);

    const getStockStatusClass = (item) => {
      if (item.quantity_on_hand <= item.reorder_point) return "danger";
      if (item.quantity_on_hand <= item.reorder_point * 1.5) return "warning";
      return "success";
    };
